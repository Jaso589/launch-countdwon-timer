...
export default function countDown(id, time, message){
    ...
    window.addEventListener('load', () => {
        ...
        function init() {
            totalSeconds = Math.floor((new Date(time) - new Date()) / 1000); 
            setTimeLeft();
            ...
            if(totalSeconds < 0){
                document.getElementById(id).innerHTML = `<h1>${message}</h1>`
            }
        }
        ...
        function animateFlip(element, value) {
            ...
            gsap.to(element.querySelector('.top'), .7, {
                rotationX: '-180deg',
